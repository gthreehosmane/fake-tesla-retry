...
export const signUp = async(req, res) => {
    ...
    var sql_insert = "INSERT INTO user (persona,fname,lname,email,pwd,signedUpOn) VALUES (?,?,?,?,?,?)";
      ...
      con.query(sql_findEmail, [email], function (err, result) {
        if (err) {
          ...
          if (result && result[0] == null) {
            /*new code start calculate number of days*/

             /*new code ends*/
            con.query(sql_insert, [persona,fname,lname, email, customerPassword,userCreationDate], function (err, result) {
              if (err) {
                res.status(205).json({
                  success:false,
